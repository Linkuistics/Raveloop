...
Answer `y` at the *Proceed to next work phase?* prompt and the loop begins again. The work phase reads a now-richer context: the same README, the now-four-entry memory, and a backlog with one task `done` and two `not_started`.

Pick the type-theory task this time:

[source,text]
----
> the type-theory entries

Working on: Add five type-theory entries on dependent typing.

[edits reading-list.md, creates notes/lof-pierce-tapl.md, ...]
...
For this hand-off, *promote* is the right call — adding implementation cross-references is concrete, scoped work that fits as its own task. After triage you should see something like:
...
| title                                                          | status      |
|----------------------------------------------------------------|-------------|
| Add first ten distributed-systems entries                      | done        |
| Add five type-theory entries on dependent typing               | done        |
...
The new task, promoted from the hand-off, is now first-class. Triage clears the `handoff` field on the original task after promoting; the original task's `Results:` block stays intact, so the audit trail still ties the new task to its origin.
